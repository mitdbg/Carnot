...
# from carnot.agents.search_agents import DataDiscoveryAgent, SearchManagerAgent
from carnot.core.data.context import Context
...
class SmolAgentsSearch(PhysicalOperator):
    ...
    def __call__(self, candidate: DataRecord) -> Any:
        ...
        cost_per_input_token = (3.0 / 1e6) if "anthropic" in self.model_id else (2.5 / 1e6) # (0.15 / 1e6) #
        cost_per_output_token = (15.0 / 1e6) if "anthropic" in self.model_id else (10.0 / 1e6) # (0.6 / 1e6) #
