...
from carnot.constants import LOCAL_SCAN_TIME_PER_KB
from carnot.core.data import context
from carnot.core.elements.records import DataRecord, DataRecordSet
from carnot.core.models import OperatorCostEstimates, RecordOpStats
...
class ContextScanOp(PhysicalOperator):
    ...
    def __call__(self, *args, **kwargs) -> DataRecordSet:
        ...
        start_time = time.time()
        dr = DataRecord(self.output_schema(context=""), source_indices=[f"{self.context.id}-{0}"])
