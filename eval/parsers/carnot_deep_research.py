"""Parser for Carnot Deep Research output.

Carnot produces intro.md (Related Works with markdown links) and paper.csv
(id, title, snippet). Same format as deepscholar_base, but implemented
as a standalone parser with citations_for_cite_quality for claim_coverage/cite_p.
"""
...
class CarnotDeepResearchParser(Parser):
    ...
    def _load_file(self):
        ...
        # Required for claim_coverage and cite_p evaluators
        if self.docs:
            self.citations_for_cite_quality = [
                (d["title"], d["sent"]) for d in self.docs
            ]
        else:
            self.citations_for_cite_quality = []
