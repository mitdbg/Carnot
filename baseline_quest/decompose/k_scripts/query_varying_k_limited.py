...
def main():
    ...
    k_values = list(range(100, 1001, 100)) + list(range(1500, 4001, 500)) # 100, 200, ..., 1000 (10 values)
    k_combinations = list(product(k_values, k_values)) # 100 total combinations
    
    OUTPUT_PATH = f"pred_query_{query_num}_varying_k_limited.jsonl"
    ...
    for k1, k2 in tqdm(k_combinations, desc=f"Executing Query {query_num} Strategy"):
        ...
        final_docs, docs_1, docs_2 = execute_query(retrieve, k1, k2)
        # --- End of Modification ---
        
        # Create the prediction dictionary
        # Now the variables match the keys correctly
        prediction = {
            "query": QUERY_TEXT, 
            "k1": k1,
            "docs_1": docs_1,
